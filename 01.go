...
func SpiderPageDb(idx int) {
	...
	if idx <= 820 {
		...
		save3File(NewsName, NewsTime, NewsAuthor, NewsWork)
	} else {
...
func save3File(NewsName, NewsTime, NewsAuthor, NewsWork [][]string) {
	n := len(NewsWork)
	filePath := "demo01.txt"
	...
	for i := 0; i < n; i++ {
		write.WriteString(NewsTime[i][1] + "\t" + NewsAuthor[i][1] + "\t\t" + NewsName[i][1] + "\n" + NewsWork[i][1] + "\n")
	}
	write.Flush()
}
